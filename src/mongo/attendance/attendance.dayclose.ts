...
// 22:00–03:59 IST window it runs the branch-wise day-close report (and, only when
// ATTENDANCE_AUTOCLOSE is set, the forgotten-checkout auto-close) — both gate themselves at 10pm
// in each BRANCH's local timezone, which is what
...
// Forgotten-checkout auto-close is OFF unless explicitly re-enabled (owner call, 09-07 — reverses
// the 07-31 owner call). Open days now stay open; the day-close report still posts every night.
const AUTO_CLOSE_ENABLED = /^(1|true|on|yes)$/i.test(process.env.ATTENDANCE_AUTOCLOSE ?? '');
...
export function startAttendanceDayClose(intervalMs = 60_000): void {
  if (timer) return;
  timer = setInterval(() => {
    // Everything here fires at 10pm in each BRANCH's local night, so the tick window must span
    // every branch's local 10pm: 22:00 IST (India) through 03:59 IST (FBM's 10pm = 01:30 IST).
    // When re-enabled via ATTENDANCE_AUTOCLOSE, autoCloseOpenDays self-gates per branch inside
    // that window and stamps the checkout at the branch's LOCAL office-end time (7pm IST India,
    // 5:30pm DAR/FBM, 6:30pm NBO), method 'Auto-closed'.
    const hour = businessHour();
    if (hour < DAY_CLOSE_HOUR && hour >= 4) return;
    const autoClose = AUTO_CLOSE_ENABLED
      ? attendanceService.autoCloseOpenDays().then((r) => {
    ...
    void autoClose
      ...
      .then(() => attendanceService.dayCloseReport())
      .catch((e: Error) => {
        // eslint-disable-next-line no-console
